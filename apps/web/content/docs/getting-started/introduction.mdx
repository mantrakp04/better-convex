...
1. **Convex Ents** - Type-safe entity relationships
2. **Zod Validation** - Runtime argument validation with `zQuery`/`zMutation`
3. **Better Auth** - Full authentication with organizations and teams
4. **Aggregates** - Real-time statistics without expensive queries
5. **Multi-Tenancy** - Organization-scoped data isolation

Continue to [Setup](/docs/getting-started/setup) to get the template running.
